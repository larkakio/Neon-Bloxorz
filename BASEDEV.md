...
1. **Primary URL** — `https://neon-bloxorz.vercel.app` (or your custom domain after you add it in Vercel). The app emits `<meta name="base:app_id" content="…" />` from [`web/src/app/layout.tsx`](web/src/app/layout.tsx) (override with `NEXT_PUBLIC_BASE_APP_ID`). **If Base.dev says “Failed to fetch URL”,** the live URL is not serving the app (e.g. Vercel **404**): fix **Vercel → Root Directory → `web`**, redeploy, open the URL in a browser until the game loads, then **Verify & Add** again (use full `https://…`).
